...
- **user_manager** is the office/team manager: it receives the user-administration permissions (`users:list`, `users:manage`, `invites:manage`) plus read-only visibility into leads, cases, and the dashboard. It does **not** get write access to cases or leads.
- **agent** is the intake / call-center role: it receives `lead:create`, `lead:view:own`, `lead:update` (note: **not** `lead:qualify`), `forms:config:view:public`, `forms:submit/background_check/npi_verify/fraud_check`, `npi:lookup`, `buyers:view`, `lead_sources:view`, `case:view:own`, `documents:view`, `calls:view`, `sms:send`, `dashboard:view`, `news:view`, `timeline:view` — scoped to its own leads only. It cannot see other agents' leads or reach cases beyond its own.
...
- An outbound calling console built on Vapi. Tabs (`dialer.tsx`): **Stats**, **Campaigns**, **Upload & Dial**, **Calls**, **Leads**, **DNC List**, **Scripts**, **Dial Pad**.
...
- **Campaigns** drive bulk outbound. Selecting a `tort` on `POST /api/dialer/campaigns` auto-links that tort's dedicated voice agent (`vapi_assistant_id`). A campaign is run by the worker job `dialer_campaign_run` (`worker.ts`): it enforces the timezone-aware call window, checks Vapi credentials, claims a batch up to `max_concurrent_calls`, applies **DNC** filtering, then places calls via `https://api.vapi.ai/call`. The worker **decrypts** each lead's `phone` (falling back to `phone_primary`) before DNC matching and dialing, since those fields are encrypted at rest.
- **Upload & Dial** (`POST /api/dialer/campaigns/upload-dial`, perms `calls:manage` **+** `leads:import:execute`) is a one-shot CSV → auto-dial bridge. Each CSV row is run through the **same lead-import pipeline** (`lib/lead-import-core.ts`: parse → auto-map columns → dedup → field encryption → conflict-check) so rows become full CRM leads. The endpoint creates a `draft` campaign (auto-picking the selected tort's voice agent), creates an import batch, and responds `202`. Asynchronously it imports the leads, links the newly-created lead IDs to the campaign, bumps `total_leads`, and — when `auto_start` is set — flips the campaign `active` and enqueues `dialer_campaign_run` so dialing begins. Bounded to 5,000 rows per upload.
- **Endpoints (`/api/dialer`, perms `calls:view`/`calls:manage`):** `GET /stats`; campaign CRUD `GET|POST /campaigns`, `POST /campaigns/upload-dial`, `GET|PATCH|DELETE /campaigns/:id`, `POST /campaigns/:id/{start,pause}`, `GET /campaigns/:id/{progress,leads}`, `POST /campaigns/:id/leads`; DNC `GET /dnc`, `POST /dnc`, `POST /dnc/bulk`, `POST /dnc/check`, `DELETE /dnc/:id`; scripts `GET|POST /scripts`, `GET|PATCH|DELETE /scripts/:id`; `POST /manual-call` (Dial Pad); `GET /vapi-config`, `POST /vapi-assistant`, `GET /vapi-phones`; `PUT /call/:id/end`.
...
- **Endpoints:**
  ...
  - `PATCH /api/users/:id/role` — admin only; body `{ role: 'user_manager'|'attorney'|'paralegal'|'agent'|'viewer' }` (`admin` and `super_admin` cannot be assigned through this route). Promotion-rule failures: §3.7. Successful change writes audit `entity_type=user, action=role_changed` with payload `{ actor_user_id, actor_email, target_email, before: { role }, after: { role }, firm_id }` and bumps `token_version` (kicks all sessions).
