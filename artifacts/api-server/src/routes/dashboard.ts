...
router.get("/stats", requirePermission(Permission.DASHBOARD_VIEW), async (req, res) => {
  try {
    ...
    logger.error({ err: err?.message }, "dashboard /stats failed");
    serverError(res, "Failed to load dashboard stats");
  }
});
...
router.get("/pipeline", requirePermission(Permission.DASHBOARD_VIEW), async (req, res) => {
  try {
    ...
    logger.error({ err: err?.message }, "dashboard /pipeline failed");
    serverError(res, "Failed to load pipeline breakdown");
  }
});
...
router.get("/recent-activity", requirePermission(Permission.DASHBOARD_VIEW), async (req, res) => {
  try {
    ...
    logger.error({ err: err?.message }, "dashboard /recent-activity failed");
    serverError(res, "Failed to load recent activity");
  }
});

// `and` is imported above because the firm-scope predicate composes with
// future date filters; keep the import explicit even if no current handler
// chains predicates yet so the diff stays small for the next change.
void and;
