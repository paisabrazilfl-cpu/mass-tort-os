...
function isTrustedFormCert(url: unknown): url is string {
...
async function runWebFormPipeline(
  ...
  // STEP 4b: TCPA / TrustedForm compliance.
  const step4b: PipelineStep = { name: "CONSENT_COMPLIANCE", status: "passed", errors: [] };
  const tcpaConsentVal = body.tcpa_consent;
  const tcpaConsented =
    tcpaConsentVal === true ||
    tcpaConsentVal === "true" ||
    tcpaConsentVal === "on";
  const trustedformCertUrl = typeof body.trustedform_cert_url === "string" ? body.trustedform_cert_url.trim() : "";
  const trustedformPingUrl = typeof body.trustedform_ping_url === "string" ? body.trustedform_ping_url.trim() : "";
  const trustedformCertToken = typeof body.trustedform_cert_token === "string" ? body.trustedform_cert_token.trim() : "";
  if (!tcpaConsented) {
    step4b.errors = ["TCPA_CONSENT_REQUIRED"];
    step4b.status = "failed";
    pipeline.push(step4b);
    return failed(pipeline, "CONSENT_COMPLIANCE", "TCPA consent is required before submitting this form");
  }
  if (!isTrustedFormCert(trustedformCertUrl)) {
    step4b.errors = ["TRUSTEDFORM_CERT_REQUIRED"];
    step4b.status = "failed";
    pipeline.push(step4b);
    return failed(pipeline, "CONSENT_COMPLIANCE", "A valid TrustedForm certificate URL is required before submitting this form");
  }
  pipeline.push(step4b);
  ...
  // TrustedForm / TCPA compliance is enforced in STEP 4b above. Carry the
  // canonical consent artifact details into persistence so every accepted web
  // lead has durable consent provenance on the lead row itself.
  const submissionIp = sourceIpOf(_req);
  const submissionUserAgent = _req.get("user-agent") ?? null;
  const trustedformTimestamp = new Date();
  ...
  try {
    ...
    if (existing) {
      ...
      setExpr.trustedform_cert_url = sql`COALESCE(${leadsTable.trustedform_cert_url}, ${trustedformCertUrl})`;
      setExpr.trustedform_ping_url = sql`COALESCE(${leadsTable.trustedform_ping_url}, ${trustedformPingUrl || null})`;
      setExpr.trustedform_cert_token = sql`COALESCE(${leadsTable.trustedform_cert_token}, ${trustedformCertToken || null})`;
      setExpr.trustedform_ip = sql`COALESCE(${leadsTable.trustedform_ip}, ${submissionIp})`;
      setExpr.trustedform_user_agent = sql`COALESCE(${leadsTable.trustedform_user_agent}, ${submissionUserAgent})`;
      setExpr.trustedform_timestamp = sql`COALESCE(${leadsTable.trustedform_timestamp}, ${trustedformTimestamp})`;
      ...
      const inserted = await db
        ...
        .values({
          ...
          trustedform_ping_url: trustedformPingUrl || null,
          trustedform_cert_token: trustedformCertToken || null,
          trustedform_ip: submissionIp,
          trustedform_user_agent: submissionUserAgent,
          trustedform_timestamp: trustedformTimestamp,
  ...
  if (leadId) {
    ...
    void enqueueSmartAdvocatePushForLead(leadId, {
      source: `web_form_${tortId}`,
    });
...
function generateWebFormEmbed(
  ...
  return `(function(){
var DATA=${json};
function isSensitiveFieldKey(key){return /(^|_)(dob|date_of_birth|birthdate|last_4_ssn|ssn_last_four|ssn)(_|$)/i.test(String(key||""));}
function installTrustedForm(){if(document.getElementById("mtos-webform-trustedform-loader"))return;var tf=document.createElement("script");tf.id="mtos-webform-trustedform-loader";tf.async=true;tf.src="https://api.trustedform.com/trustedform.js?field=xxTrustedFormCertUrl&ping_field=xxTrustedFormPingUrl&use_tagged_consent=true&l="+(new Date().getTime())+Math.random();var first=document.getElementsByTagName("script")[0];if(first&&first.parentNode)first.parentNode.insertBefore(tf,first);else(document.head||document.body||document.documentElement).appendChild(tf);}
...
function input(f){
  if(f.type==="textarea"){var ta=el("textarea",{name:f.key,id:"wf_"+f.key,rows:"4",maxlength:String(f.max_length||2000),placeholder:f.placeholder||""});if(isSensitiveFieldKey(f.key))ta.setAttribute("data-tf-sensitive","true");return ta;}
  ...
  if(f.type==="select"||f.type==="state"){
    ...
    if(isSensitiveFieldKey(f.key))s.setAttribute("data-tf-sensitive","true");
    return s;
  }
  if(f.type==="radio"){
    ...
    (f.options||[]).forEach(function(o,i){
      ...
      if(isSensitiveFieldKey(f.key))r.setAttribute("data-tf-sensitive","true");
      lab.appendChild(r);
  ...
  var inp=el("input",{type:f.type,name:f.key,id:"wf_"+f.key,maxlength:String(f.max_length||255),placeholder:f.placeholder||""});
  if(isSensitiveFieldKey(f.key))inp.setAttribute("data-tf-sensitive","true");
  return inp;
}
...
function init(){
  ...
  var consentField=null;
  DATA.fields.forEach(function(f){if(f.key==="tcpa_consent")consentField=f;});
  var sections=[["eligibility","Eligibility"],["contact","Your Contact Info"],["story","Your Situation"]];
  sections.forEach(function(s){
    var fs=DATA.fields.filter(function(f){return f.section===s[0]&&f.key!=="tcpa_consent";});
  ...
  var offerFieldset=null;
  if(consentField){
    offerFieldset=el("fieldset",{"class":"wf-section wf-section-consent","data-tf-element-role":"offer"});
    offerFieldset.appendChild(el("legend",{text:"Consent & Submission"}));
    var consentRow=el("div",{"class":"wf-row"});
    var consentInline=el("div",{"class":"wf-checkbox-row"});
    var consentBox=input(consentField);
    consentBox.setAttribute("required","");
    consentBox.setAttribute("data-tf-element-role","consent-opt-in");
    var consentLang=el("span",{"class":"wf-cb-label","data-tf-element-role":"consent-language",text:consentField.label});
    consentInline.appendChild(consentBox);
    consentInline.appendChild(consentLang);
    consentRow.appendChild(consentInline);
    consentRow.appendChild(el("div",{"class":"wf-error","id":"wfe_tcpa_consent"}));
    offerFieldset.appendChild(consentRow);
    offerFieldset.appendChild(el("input",{type:"hidden",name:"xxTrustedFormCertUrl",id:"xxTrustedFormCertUrl_0",value:""}));
    offerFieldset.appendChild(el("input",{type:"hidden",name:"xxTrustedFormPingUrl",id:"xxTrustedFormPingUrl_0",value:""}));
    offerFieldset.appendChild(el("input",{type:"hidden",name:"xxTrustedFormCertToken",id:"xxTrustedFormCertToken_0",value:""}));
    form.appendChild(offerFieldset);
  }
  var msg=el("div",{"class":"wf-msg"});
  var btn=el("button",{type:"submit","data-tf-element-role":"submit"});
  var btnText=el("span",{"data-tf-element-role":"submit-text",text:"Submit"});
  btn.appendChild(btnText);
  (offerFieldset||form).appendChild(msg);
  (offerFieldset||form).appendChild(btn);
  form.addEventListener("submit",function(ev){
    ...
    if(prefVal!=="agent"&&prefVal!=="text_email"){
      ...
      btn.disabled=false;btnText.textContent="Submit";
      return;
    }
    ...
    var tfToken=document.getElementById("xxTrustedFormCertToken_0");
    if(tfToken&&tfToken.value)payload.trustedform_cert_token=tfToken.value;
