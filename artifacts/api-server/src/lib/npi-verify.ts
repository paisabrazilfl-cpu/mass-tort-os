...
function pickBestSearchResult(
  ...
  // Hoist normalization of expected fields outside the candidate loop to avoid up to 96% of redundant calls
  ...
  for (const r of results) {
    ...
    // Pre-normalize current candidates in the loop
...
function specialtyAcceptedTerms(expectedSpecialty: string): string[] {
  ...
  // Since specialty aliases in SPECIALTY_ALIASES are statically defined lowercase strings
  // that do not contain special punctuation, we can bypass normalize() for them to avoid overhead.
