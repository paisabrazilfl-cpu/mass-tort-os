...
function pickBestSearchResult(
  ...
  // Pre-normalize expected fields to avoid $O(N)$ redundant processing
  ...
  for (const r of results) {
    ...
    // Identity scoring (Name / Organization)
    const provNameNorm = normalize(name);
    const provOrgNorm = normalize(org);

    const nameScore = Math.max(
      similarityNamePreNormalized(expNameNorm, provNameNorm, expNameStripped),
      similarityNamePreNormalized(expNameNorm, provOrgNorm, expNameStripped),
    );
    const orgScore = similarityPreNormalized(expOrgNorm, provOrgNorm);

    // Location scoring
    const provCityNorm = normalize(primaryAddr.city ?? "");
    const provStateNorm = normalize(primaryAddr.state ?? "");

    const cityScore = similarityPreNormalized(expCityNorm, provCityNorm);
    const stateScore =
