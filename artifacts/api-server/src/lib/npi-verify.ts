...
// Hoisted and normalized static specialty alias arrays to avoid redundant normalizations
const NORMALIZED_SPECIALTY_ALIASES: Record<string, readonly string[]> = {};
for (const [key, list] of Object.entries(SPECIALTY_ALIASES)) {
  NORMALIZED_SPECIALTY_ALIASES[key] = list
    .map((t) => normalize(t))
    .filter(Boolean);
}
...
function pickBestSearchResult(
  ...
  // Hoisted and pre-normalized expected fields outside the search loop!
  ...
  for (const r of results) {
    ...
    if (score > bestScore) {
      bestScore = score;
      best = r;
      // Early-exit for an absolute perfect candidate to bypass subsequent loop cycles
      if (score >= 1.0) {
        break;
      }
    }
...
function specialtyAcceptedTerms(expectedSpecialty: string): string[] {
  ...
  if (!base) return [];
  const aliases = NORMALIZED_SPECIALTY_ALIASES[base] ?? [];
