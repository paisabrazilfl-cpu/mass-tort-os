...
function pickBestSearchResult(
  ...
  // Hoist normalization of target fields out of the search loop to avoid
  // redundant regex/lowercasing on every candidate iteration.
  const targetNameNorm = normalize(expected.name);
  const targetOrgNorm = normalize(expected.organization);
  const targetCityNorm = normalize(expected.city);
  const targetStateNorm = normalize(expected.state);
  ...
  for (const r of results) {
    ...
    const candOrg = basic.organization_name ?? "";
    const primaryAddr = pickPrimaryAddress(r.addresses);

    // Pre-normalize candidate fields once to reuse in multiple comparisons
    const candNameNorm = normalize(candName);
    const candOrgNorm = normalize(candOrg);
    const candCityNorm = normalize(primaryAddr.city);
    const candStateNorm = normalize(primaryAddr.state);

    const nameScore = Math.max(
      similarityNamePreNormalized(targetNameNorm, candNameNorm),
      similarityNamePreNormalized(targetNameNorm, candOrgNorm),
    );
    const orgScore = similarityPreNormalized(targetOrgNorm, candOrgNorm);
    const cityScore = similarityPreNormalized(targetCityNorm, candCityNorm);
    const stateScore =
      targetStateNorm === candStateNorm
        ? 1.0
        : similarityPreNormalized(targetStateNorm, candStateNorm);
    ...
    if (bestScore >= 1.0) break; // Early return for perfect match
  }
