...
function pickBestSearchResult(
  ...
  for (const r of results) {
    ...
    // Early-exit matching: if the best possible score with name/org is exact, and we can shortcut.
...
function specialtyAcceptedTerms(expectedSpecialty: string): string[] {
  ...
  // The aliases inside SPECIALTY_ALIASES are static arrays that could contain mixed cases,
  // but let's normalize them. Since we only have a few hardcoded, we can filter/normalize cleanly.
  const all = [base];
