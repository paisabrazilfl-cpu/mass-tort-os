...
function pickBestSearchResult(
  ...
  const nExpOrg = normalize(expected.organization ?? "");
  const nExpCity = normalize(expected.city ?? "");
  const nExpState = normalize(expected.state ?? "");
  ...
  for (const r of results) {
    ...
    const nCity = normalize(primaryAddr.city ?? "");
    const nState = normalize(primaryAddr.state ?? "");

    const nameScore = Math.max(
      similarityNamePreNormalized(nExpName, nName),
      similarityNamePreNormalized(nExpName, nOrg),
    );
    const orgScore = similarityPreNormalized(nExpOrg, nOrg);
    const cityScore = similarityPreNormalized(nExpCity, nCity);
    const stateScore =
      nExpState === nState ? 1.0 : similarityPreNormalized(nExpState, nState);
