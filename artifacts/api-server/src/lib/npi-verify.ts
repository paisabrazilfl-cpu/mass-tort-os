...
function pickBestSearchResult(
  ...
  // Hoist normalization out of the search loop
  ...
  for (const r of results) {
    const basic = r.basic ?? {};
    const nameNorm = normalize(
      basic.name ?? (basic.first_name || basic.last_name ? `${basic.first_name || ""} ${basic.last_name || ""}` : ""),
    );
    const orgNorm = normalize(basic.organization_name ?? "");
    ...
    if (score > bestScore) {
      bestScore = score;
      best = r;
      // Early break if we found a perfect provider match (identity + location)
...
function compareStringsField(
  ...
  const score = similarityPreNormalized(
    normalize(providerValue ?? ""),
    expectedNormalized ?? normalize(expectedValue),
  );
...
export async function verifyProvider(
  ...
  const expOrgNorm = normalize(expected.organization ?? "");
  const orgCheck = compareStringsField(
    provOrg,
  ...
  const cityCheck = compareStringsField(
    provCity,
    expected.city,
    0.85,
    expCityNorm,
