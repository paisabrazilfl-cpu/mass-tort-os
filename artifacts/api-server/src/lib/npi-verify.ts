...
function pickBestSearchResult(
  ...
  // Pre-normalize expected fields once to avoid redundant work in the loop
  ...
  for (const r of results) {
    ...
    const name = basic.name ?? `${first || ""} ${last || ""}`.trim();
    const org = basic.organization_name ?? "";
    const primaryAddr = pickPrimaryAddress(r.addresses);

    // Reuse pre-normalized values and specialized helpers to minimize overhead
    const nameScore = Math.max(
      similarityNamePreNormalized(expNameNorm, normalize(name)),
      similarityNamePreNormalized(expNameNorm, normalize(org)),
    );
    const orgScore = similarityPreNormalized(expOrgNorm, normalize(org));
    const cityScore = similarityPreNormalized(expCityNorm, normalize(primaryAddr.city));

    const provStateNorm = normalize(primaryAddr.state);
    const stateScore =
      expStateNorm === provStateNorm
        ? 1.0
        : similarityPreNormalized(expStateNorm, provStateNorm);
    ...
    if (score > bestScore) {
      bestScore = score;
      best = r;
      // Early exit on perfect match — impossible to beat
...
function compareStringsField(
  ...
  expectedValueNormalized: string, // Accept pre-normalized expected value
  threshold = 0.8,
): { match: boolean; score: number } {
  if (!expectedValueNormalized) return { match: true, score: 1.0 };
  const score = similarityPreNormalized(normalize(providerValue), expectedValueNormalized);
...
function summarizeProvider(p: NpiRegistryResult): ProviderSummary {
  ...
  return {
    npi: String(p.number ?? ""),
    name: basic.name ?? `${first || ""} ${last || ""}`.trim(),
...
export async function verifyProvider(
  ...
  const orgCheck = compareStringsField(provOrg, expOrgNorm, 0.7);
  ...
  const cityCheck = compareStringsField(provCity, expCityNorm, 0.85);

  const provStateNorm = normalize(provState);
  const stateExact = provStateNorm === expStateNorm;
  const stateScore = stateExact
    ? 1.0
    : similarityPreNormalized(provStateNorm, expStateNorm);
