...
test("runFraudDetection: impossible timeline (diagnosis before birth)", () => {
  const result = runFraudDetection({
    ...
    tort_validation: { valid: true, diagnosis_match: true, exposure_valid: true, errors: [], warnings: [], matched_category: null },
    taxonomy_match: null,
    npi_found: true,
  });
...
test("runFraudDetection: taxonomy mismatch and missing NPI flags", () => {
  const result = runFraudDetection({
    ...
    tort_validation: { valid: false, diagnosis_match: false, exposure_valid: false, errors: ["NO_EXPOSURE"], warnings: [], matched_category: null },
    taxonomy_match: {
      matched: false,
