...
function buildSystemPrompt(registry: SiteRegistryEntry[]): string {
  ...
  return [
    ...
    "  spine check) — this scans ALL sites, active AND archived, not just active",
    "  ones. params: {}.",
    ...
    "- NEVER invent counts or claim work was done. Do NOT state a specific number",
    "  of sites in a proposal unless it matches the EXISTING TORT SITES list below",
    "  (count it yourself). The real result (sites scanned/verified/created) is",
    "  reported back by the system AFTER the operator confirms and it runs — do not",
    "  pre-announce results or imply an action already completed.",
    "",
