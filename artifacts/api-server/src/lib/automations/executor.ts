...
export const HANDLERS: Record<string, (s: StepContext) => Promise<HandlerResult>> = {
  ...
  "script.javascript": async (s) => {
    ...
    try {
      ...
      throw new Error(`script.javascript: ${err instanceof Error ? err.message : String(err)}`);
    }
  ...
  "ai.agent": async (s) => {
    ...
    try {
      ...
      return { __branch: "error", value: { error: err instanceof Error ? err.message : String(err) } };
    }
  },
  "ai.classify": async (s) => {
    const p = s.node.data?.params ?? {};
