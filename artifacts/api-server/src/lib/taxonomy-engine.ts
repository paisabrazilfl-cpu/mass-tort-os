...
/**
 * Direct NPI-by-number lookup against the NPPES public registry.
 * Accepts a 10-digit NPI string and returns the same shape as
 * `lookupNpiAndMatch`, with the registered name fields included so
 * downstream automations can verify the operator-supplied name.
 *
 * Used by the `crm.npi_lookup` automation node when the operator passes
 * a raw NPI number (previously this path returned `lookup: "deferred"`).
 */
export async function lookupNpiByNumber(
  ...
  const clean = String(npiNumber || "").replace(/\D/g, "");
  if (!/^\d{10}$/.test(clean)) {
    return {
      npi_found: false,
      npi_number: null,
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        specialty: null,
        taxonomy_match: null,
        error: "NPI_API_UNAVAILABLE",
      };
    }
    const data = (await response.json()) as {
      results?: Array<{
        number?: number;
        basic?: { first_name?: string; last_name?: string; name?: string };
    ...
    const specialty = primaryTax.desc || "Unknown";
    const taxonomyCode = primaryTax.code || undefined;
    const firstName = provider.basic?.first_name ?? null;
    const lastName = provider.basic?.last_name ?? null;
    return {
      npi_found: true,
      npi_number: String(provider.number ?? clean),
      first_name: firstName,
      last_name: lastName,
      specialty,
      taxonomy_match: matchTaxonomyToDiagnosis(specialty, diagnosis, taxonomyCode),
      error: null,
    };
  } catch (err) {
    logger.warn({ err, npiNumber: clean }, "NPI-by-number lookup failed");
    return {
      npi_found: false,
      npi_number: null,
      first_name: null,
