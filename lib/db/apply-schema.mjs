...
// Orphaned tables to drop from already-provisioned databases. These backed the
// removed Sites AI Assistant feature and are no longer in the Drizzle schema.
// On a *fresh* DB the DROP SCHEMA below already removes everything, so this list
// only matters for existing databases where the sentinel short-circuits the
// full bootstrap. Order matters: drop dependents before their parents (messages
// has an FK to conversations). DROP ... IF EXISTS CASCADE makes this idempotent.
const ORPHANED_TABLES = ["messages", "conversations"];
...
async function main() {
  ...
  try {
    ...
    if (schemaPresent) {
      ...
      // Even on a provisioned DB, sweep any orphaned tables left behind by
      // removed features. Idempotent: a no-op once they're gone.
      for (const table of ORPHANED_TABLES) {
        const { rows: present } = await client.query(
          "SELECT to_regclass($1) AS reg",
          [`public.${table}`],
        );
        if (present[0]?.reg != null) {
          log(`dropping orphaned table ${table}.`);
