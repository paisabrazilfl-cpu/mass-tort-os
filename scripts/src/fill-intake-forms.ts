/**
 * fill-intake-forms.ts
 *
 * Replaces every tort's web_form_config with a comprehensive, deep-qualifying
 * intake form. Each form has:
 *   - Eligibility section: hard knockout gates (radio Yes/No)
 *   - Contact section:     standard 6 contact fields
 *   - Story section:       15-20 tort-specific deep-qualifying questions
 *
 * Run:  pnpm --filter @workspace/scripts exec tsx src/fill-intake-forms.ts
 */
import { db, formConfigurationsTable } from "@workspace/db";
import { eq } from "drizzle-orm";
import type { WebFormConfig, WebFormField, EligibilityRule } from "@workspace/db/schema";
...
const CONTACT_FIELDS: WebFormField[] = [
  ...
  { key: "phone", label: "Phone number", type: "tel", section: "contact", required: true, placeholder: "555-555-5555" },
  { key: "dob", label: "Date of birth", type: "date", section: "contact", required: true },
...
const BASE_ELIGIBILITY: WebFormField[] = [
  ...
  { key: "represented_by_attorney", label: "Are you currently represented by an attorney for this specific matter?", type: "radio", section: "eligibility", required: true, options: ["Yes", "No"] },
];
...
const BASE_RULES: EligibilityRule[] = [
  ...
  { id: "tcpa_required", field: "tcpa_consent", op: "ne", value: "true", message: "You must consent to be contacted in order to submit this form." },
];
...
function conf(
  ...
  const config: WebFormConfig = {
    ...
    eligibility_rules: [...BASE_RULES, ...rulesExtra],
...
// ─── Per-tort comprehensive form definitions ────────────────────────────────

const FORMS: [string, WebFormConfig][] = [
  ...
  conf("roundup",
    ...
    [
      ...
      { key: "prior_roundup_claim", label: "Have you previously filed or settled a claim related to Roundup?", type: "radio", section: "story", required: true, options: ["Yes", "No"] },
    ]
  ),
  ...
  conf("talcum-powder",
    ...
    [
      { key: "used_talcum", label: "Did you use Johnson's Baby Powder, Shower to Shower, or another talcum-based product in the genital/perineal area?", type: "radio", section: "eligibility", required: true, options: ["Yes", "No", "Unsure"] },
    ...
    [
      ...
      { key: "prior_talcum_claim", label: "Have you previously filed a claim related to talcum powder?", type: "radio", section: "story", required: true, options: ["Yes", "No"] },
    ]
  ),
  ...
  conf("paraquat",
    ...
    [
      ...
      { key: "paraquat_prior_claim", label: "Have you previously filed a claim related to Paraquat?", type: "radio", section: "story", required: true, options: ["Yes", "No"] },
    ]
  ),
  ...
  conf("hair-relaxer",
    ...
    [
      { key: "relaxer_brands", label: "Which hair relaxer brand(s) did you use? (select all that apply)", type: "select", section: "story", required: true, options: ["Dark & Lovely", "Just for Me", "ORS Olive Oil", "Motions", "Soft & Beautiful", "Revlon Realistic", "PCJ", "Affirm", "Multiple brands / store brands", "Other / don't recall brand"] },
  ...
  conf("social-media",
    ...
    [
      { key: "claimant_type", label: "Who is affected by social media harm?", type: "radio", section: "eligibility", required: true, options: ["I am the affected person (age 18+ now)", "I am the parent/guardian of a minor who was affected", "I am filing on behalf of an adult child"] },
    ...
    [
      ...
      { key: "diagnoses", label: "What mental health diagnosis(es) has been received?", type: "select", section: "story", required: true, options: ["Clinical depression", "Anxiety disorder", "Eating disorder (anorexia, bulimia, ARFID)", "Body dysmorphia", "PTSD or trauma", "Self-harm behaviors", "Suicide attempt", "Addiction to social media (diagnosed)", "Multiple diagnoses"] },
  ...
  conf("roblox",
    ...
    [
      ...
      { key: "nature_of_harm", label: "What type of harm occurred? (select all that apply — describe sensitive details in the description field)", type: "select", section: "story", required: true, options: ["Grooming by an adult stranger", "Sexual solicitation through chat", "Exploitation involving images or videos", "In-person meeting arranged through platform", "Financial exploitation / manipulation", "Cyberbullying and harassment", "Multiple types of harm"] },
  ...
  conf("online-gaming",
    ...
    [
      ...
      { key: "diagnosis", label: "Mental health or medical diagnosis received", type: "select", section: "story", required: false, options: ["Internet Gaming Disorder (IGD)", "Depression", "Anxiety", "ADHD exacerbated by gaming", "Sleep disorder", "No formal diagnosis yet", "Other"] },
  ...
  conf("rideshare-assault",
    ...
    [
      ...
      { id: "must_be_within_sol", field: "assault_within_sol", op: "eq", value: "No", message: "Due to statutes of limitations, we typically need to have been within the last 10 years. Please consult a local attorney for older incidents." },
    ],
    [
...
async function main() {
  console.log(`Updating ${FORMS.length} form configurations with comprehensive intake questions...`);
  let updated = 0;
  let skipped = 0;

  for (const [tortId, config] of FORMS) {
    const result = await db
      .update(formConfigurationsTable)
      .set({ web_form_config: config as any })
      .where(eq(formConfigurationsTable.id, tortId));
    // @ts-ignore rowCount exists on postgres driver result
    if (result?.rowCount > 0 || (result as any)?.length > 0) {
      updated++;
      console.log(`  ✓ ${tortId} (${config.fields.length} fields)`);
    } else {
      skipped++;
      console.log(`  - ${tortId} not found, skipping`);
    }
  }
