...
*   **Role-Based Access Control (RBAC)**: JWT authentication with role hierarchy (admin > attorney > paralegal > viewer). Auth routes: `/api/auth/login`, `/api/auth/register`, `/api/auth/me`. `mtos_users` DB table.
