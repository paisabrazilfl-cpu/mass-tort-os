...
*   **T006 — RBAC + OpenAPI + verify scripts + gates**: New `Permission.{BILLING_MANAGE, CALLS_VIEW, SMS_SEND}` plus role-matrix update. New billing/calls/SMS routes added to `lib/api-spec/openapi.yaml`; codegen regenerated and consumed by `pages/{billing,calls}.tsx` via the generated React Query hooks (no ad-hoc fetch). Operator scripts `artifacts/api-server/src/scripts/verify-{stripe,vapi}.ts` mirror the docusign pattern: take an integration row id, decrypt the vault, ping the live API. `verify-stripe.ts` deliberately exits non-zero when the webhook signing secret (`whsec_...`) is missing or malformed — without it `firms.subscription_status` would never advance and the subscription gate would block all writes. **Round-5 firm-context layer**: `AuthUser.firm_id` is required, `generateToken()` embeds it, `_authMiddleware` fail-closes on missing claim, and a new `firmContextMiddleware` (lib/firm-context.ts) loads `req.firm` immediately after auth — every fail-close path writes an `audit_log` row (`firm_context_no_user` / `firm_context_missing_claim` / `firm_context_not_found`). Calls list endpoint + UI gained inclusive `start_date`/`end_date` filters against `call_logs.started_at`. **Round-6 credential-model + codegen polish**: Vapi and Telnyx integration presets now declare `["api_key", "client_secret"]` so the existing integrations form renders the second field; preset `notes` document the exact mapping (Vapi: api_key = server key + HMAC secret, client_secret = tool-callback bearer; Telnyx: api_key = API v2 key, client_secret = webhook Ed25519 public key). `send-sms-button.tsx` now consumes the generated `useSendLeadSms()` mutation hook instead of `apiFetch`, completing the OpenAPI-codegen rule for every new write endpoint. All four standing gates (typecheck, rbac-test 133/133, rbac-route-matrix, db-drift) green at sign-off.
*   **Task #58 — Admin Users page (viewer→attorney/paralegal elevation)**: New `Permission.USERS_MANAGE` (admin-only). New `routes/users.ts` exposes `GET /api/users` (firm-scoped list, USERS_LIST) and `PATCH /api/users/:id/role` (USERS_MANAGE). Self-edit returns 403 `cannot_change_own_role` to prevent a sole admin from accidentally locking the firm out; cross-firm targets return 404 (no existence oracle). Role updates are atomic with a `token_version` bump so any in-flight JWTs from the demoted user are invalidated on the next protected request. All deny paths write `audit_log` rows. New `pages/users.tsx` mirrors `firm-settings.tsx` (`apiFetchRaw` + table + dialog + role select); the route is exposed under Configuration in the sidebar and gated behind `isAdmin`. `mtos_users.last_login_at` column added and stamped by `/auth/login` on success (gives the operator a "stale account" signal in the table). Tests: `routes/__tests__/users-route.test.ts` (5 cases) covers list scoping, viewer 403, role+token_version atomic bump, self-edit 403, and cross-firm 404 — added to the rbac-test workflow. **Accepted divergence (continuing the precedent set in Task #57 `firm-invites`)**: this admin-only management endpoint uses `apiFetchRaw` instead of OpenAPI codegen because it is not consumed by external callers and adding it to the spec would gate the fix on a codegen round-trip. All four standing gates green at sign-off (typecheck, rbac-test 149/149, rbac-route-matrix 201 routes, db-drift).
